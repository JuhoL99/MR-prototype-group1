    ...
    public class CharacterImporterLogic
    {
        ...
        public async Task DownloadCharacter(string characterID)
        {
            ...
            try
            {
                ...
                //string path = $"{DirectoryUtility.GetRelativeProjectPath(args.Avatar.name, AvatarCache.GetAvatarConfigurationHash(avatarLoaderSettings.AvatarConfig))}/{args.Avatar.name}";
                string path = Application.persistentDataPath; //fix
